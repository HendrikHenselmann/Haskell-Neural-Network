...
applyToMatColWise fnc mat
    ...
    where
        ...
        aux mat j
            | (j >= (m mat)) = []
            | otherwise = (applyToMatColWise_Aux fnc mat j):(aux mat (j+1))

-- folding column j
applyToMatColWise_Aux :: (Double -> Double -> Double) -> Matrix -> Int -> Double
applyToMatColWise_Aux fnc mat j = aux fnc mat 1 j first
    where
        first = get mat 0 j
